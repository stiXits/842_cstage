...
int runTest( int argc, const char* argv[])
{
    ...
    if(argc <= 1) {
        ...
        in = (uint8_t*) malloc(ilen * sizeof(char));
        out = (uint8_t*) malloc(olen * sizeof(char));
        decompressed = (uint8_t*) malloc(dlen * sizeof(char));
...
#pragma omp parallel for
        for(int chunk_num = 0; chunk_num < num_chunks; chunk_num++) {
            ...
            uint8_t* chunk_in = in + (BLOCK_SIZE * chunk_num);
            uint8_t* chunk_out = out + ((BLOCK_SIZE * 2) * chunk_num);


#ifdef USEHW
            hw842_compress(chunk_in, chunk_out);
#else
...
#pragma omp parallel for
        for(int chunk_num = 0; chunk_num < num_chunks; chunk_num++) {
            ...
            uint8_t* chunk_in = in + (BLOCK_SIZE * chunk_num);
            uint8_t* chunk_condensed = out_condensed + compressedChunkPositions[chunk_num];
            uint8_t* chunk_decomp = in + (BLOCK_SIZE * chunk_num);


#ifdef USEHW
            hw842_decompress(chunk_condensed, chunk_decomp);
#else
...
#ifdef USEHW
        hw842_compress(in, out);
#else
        sw842_compress(in, out, BLOCK_SIZE);
#endif

#ifdef USEHW
        hw842_decompress(out, decompressed);
#else
